...
- Create-button label on the reservations page is SPLIT BY ROLE: staff sees « Nouvelle réservation » (`reservations.add`), adherent sees « Nouvelle demande de réservation » (`reservations.add.self` + `.title.self` for the modal title) — member labels always say « demande », never plain « réservation ».
- Modal submit-button labels are also role-split: reservation modal staff = « Enregistrer la réservation » (`reservations.submit`) / adherent = « Envoyer la demande de réservation » (`reservations.submit.self`); borrow modal staff = « Enregistrer l'emprunt » (NOT « Valider l'emprunt » — that wording is reserved for the workflow validation action in table rows, detail modals and confirm dialogs) / adherent = « Envoyer la demande d'emprunt ».
- `DELETE /borrow/{id}`: Admin-only. Safe-delete mirrors book deletion — deleting an in-progress borrow (VALIDEE, no returnDate) is a 409 (the decremented copy would be lost); returned/refused borrows delete fine. UI: borrow-book has ONE unified admin table (all borrows, incl. pending requests) with per-row actions — details (modal), valider/refuser/retourner (generic confirm modal), supprimer (chained: confirm modal → typed-book-name modal for active borrows/requests; SINGLE simple confirm modal for RENDU/REFUSEE — no typed-name step, per user request). Pending requests (EN_ATTENTE) appear directly in the main borrow table, filterable via the statut `<select>` (same UX as reservations, where DEMANDE rows show in the same list). NO toggle button, NO folded requests section, NO dedicated page — the "Voir toutes les demandes d'emprunt" button, the folded section and the `/pending-borrows` page were ALL removed at the user's request (asked twice — do NOT re-add any of them). No borrow count in the main table heading. The old "Tous les emprunts" heading is REPLACED by a statut filter `<select>` (Tous les emprunts + 5 statuts), sitting in the same `.table-actions-bar` row as the buttons (same height, flex layout). ADHERENT-facing labels consistently say "demande" (Demande d'emprunt, Demande de réservation) — never "Emprunter/Réserver" — because members can only submit requests, never act directly. Sidebar keys are SPLIT by role: member section uses `sidebar.reserve` ("Mes réservations"), `sidebar.my-borrows` ("Mes emprunts"), `sidebar.my-returns` ("Mes retours"); staff Gestion section uses `sidebar.reservations` ("Reservation"), `sidebar.borrow` ("Emprunt"), `sidebar.return` ("Rendre") — same `/reservations`, `/borrow-book`, `/return-book` routes for both roles. Page headers are management-style for staff AND members: `/borrow-book` = "Gestion des emprunts — Créer, consulter et annuler des emprunts" (`borrow.title`/`borrow.subtitle`), `/return-book` = "Gestion des retours…" (`return.title`/`return.subtitle`) — no per-role conditional subtitle on borrow-book.
...
- **Borrow/return forms**: "Nouvel emprunt" / "Nouveau retour" open modal forms (`borrow-book` / `return-book` components). Staff pick a member + resource (borrowing for anyone, immediate VALIDEE); adherents never send `userId` in the body — the backend takes identity from the JWT (RS-04). Book picker in the "Nouvel emprunt" modal = **predictive autocomplete input for EVERYONE** (staff AND adherents — the old staff `<select>` was replaced at the user's request): typing filters the FULL catalogue (incl. 0-copy books), selecting a 0-copy book shows "indisponible — Le réserver" and an unknown name shows "Aucun livre — Le réserver quand même", both routing to `/reservations?reserve=<id>` (existing book) or `/reservations?reserveName=<nom>` (unknown → `newBookName`). Staff picks the member first (select), then the book; the reserve-fallback hints are NOT gated on role (`unavailableBook`/`unknownBookQuery` work for staff too). Submit is blocked unless the selected book has copies and isn't already borrowed. `borrow-book` also accepts `?book=<id>` to prefill the modal (linked from book-details "Emprunter"). The borrow page has NO catalog table anymore — it was removed at the user's request (books are browsable on the Livres page; the modal's book picker covers selection). Do NOT re-add a books listing table to borrow-book, and no `scrollToCatalog`/`openBorrowModalWithBook`/direct `borrowBook()` helpers. The borrow table rows are NOT clickable — details open ONLY via the eye icon (user request, do not re-add row-click).
...
- **Assignment tests (Partie 2)**: RG-03 unit test = `ReservationServiceQuotaTest` (Mockito, repo mocked, no DB: 2 active → 3rd OK, 3 active → 409 ConflictException, quota counts DEMANDE+EN_ATTENTE+DISPONIBLE). Integration test on secured endpoint = `ReservationSecurityIntegrationTest` (real JWT + filter + @PreAuthorize, repos mocked so no DB needed): 401 without token, 200 ADHERENT sees only own (RS-05), 403 other's reservation (RS-03), plus RS-04 proof test (body userId=77 forged by alice/id-10 → created with userId=10 from token) and a staff-creation test (BIBLIOTHECAIRE reserves FOR an adherent → statut directly EN_ATTENTE, not DEMANDE). Run via plain `./mvnw test` — 39/39 green.
